...
var starckFilmes = IndexerMeta{
	Label:       "starck_filmes",
	URL:         utils.GetIndexerURLFromEnv("INDEXER_STARCK_FILMES_URL", "https://starckfilmes.org/"),
	SearchURL:   "?s=",
	PagePattern: "page/%s",
}
...
func (i *Indexer) HandlerStarckFilmesIndexer(w http.ResponseWriter, r *http.Request) {
	...
	indexedTorrents := utils.ParallelFlatMap(links, func(link string) ([]schema.IndexedTorrent, error) {
		return getTorrentsStarckFilmes(ctx, i, link, url)
	})
...
func getTorrentsStarckFilmes(ctx context.Context, i *Indexer, link, referer string) ([]schema.IndexedTorrent, error) {
	...
	title = strings.ReplaceAll(title, "Starck Filmes", "")
	title = strings.ReplaceAll(title, "StarckFilmes", "")
