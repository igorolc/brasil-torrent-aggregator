...
// findQualityFromTitle extracts quality information (e.g., 4K, 1080p) from a title string.
func findQualityFromTitle(title string) string {
	res := ""
	uTitle := strings.ToUpper(title)
	if strings.Contains(uTitle, "2160") || strings.Contains(uTitle, "4K") || strings.Contains(uTitle, "UHD") {
		res = "2160p 4K"
	} else if strings.Contains(uTitle, "1080") {
		res = "1080p"
	} else if strings.Contains(uTitle, "720") {
		res = "720p"
	}
	return res
}

// findQualityFromText extracts quality labels from a multi-line text (e.g., web-dl, blu-ray).
func findQualityFromText(text string) string {
	uText := strings.ToUpper(text)
	quality := ""
	if strings.Contains(uText, "4K") || strings.Contains(uText, "2160") {
		quality = "2160p 4K"
	} else if strings.Contains(uText, "1080") {
		quality = "1080p"
	} else if strings.Contains(uText, "720") {
		quality = "720p"
	}

	// Capture source labels
	source := ""
	if strings.Contains(uText, "BLU-RAY") || strings.Contains(uText, "BLURAY") || strings.Contains(uText, "BDRIP") {
		source = "BluRay"
	} else if strings.Contains(uText, "WEB-DL") || strings.Contains(uText, "WEBDL") || strings.Contains(uText, "WEB DX") {
		source = "WEB-DL"
	} else if strings.Contains(uText, "CAM") || strings.Contains(uText, "TS") || strings.Contains(uText, "HDTC") {
		source = "CAM/TS"
	}
...
// CleanSearchQuery removes year patterns (1900-2099) from the search query.
func CleanSearchQuery(q string) string {
	// Pattern for 4-digit years (1900-2099)
	yearRe := regexp.MustCompile(`\b(19|20)\d{2}\b`)
	res := yearRe.ReplaceAllString(q, "")
