...
// CleanTitle removes common advertisements, language markers and noise from titles, padronizando com pontos para o Radarr.
func CleanTitle(title string) string {
	// Remove common marketing suffixes and noise
	noise := []string{
		"comando.la", "REDE", "REDE TORRENT", "REDE TORRENTS",
		"HidraTorrents", "Bludv", "VacaTorrent", "StarckFilmes",
		"TorrentDosFilmes", "Download", "Baixar", "brazilian", "eng", "english", "portugues", "portuguese",
	}
	...
	// Remove language markers and brackets
	langRegex := regexp.MustCompile(`(?i)(\(brazilian\)|\(eng\)|\(english\)|\(português\)|\(portugues\)|\[dublado\]|\[legendado\]|dual audio|dual áudio)`)
	res = langRegex.ReplaceAllString(res, "")
	...
	// Clean up whitespaces and join with dots
	words := strings.Fields(res)
	return strings.Join(words, ".")
}

// CleanOriginalTitle removes metadata pollution from extracted original titles.
func CleanOriginalTitle(title string) string {
	res := title
	stops := []string{
		"IMDb", "Lançamento", "Gênero", "Genero", "Ano",
		"Qualidade", "Formato", "Áudio", "Audio", "Legenda", 
		"Tamanho", "Duração", "Direção", "Sinopse",
	}

	for _, stop := range stops {
		if idx := strings.Index(res, stop); idx != -1 {
			res = res[:idx]
		}
	}
